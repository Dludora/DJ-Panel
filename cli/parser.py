...
from app.execution.definitions import DEFAULT_DJ_BIN, WorkerMode
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run_submit.add_argument(
        "--kind",
        default=RunSubmissionKind.PROCESSING_PIPELINE.value,
        choices=[kind.value for kind in RunSubmissionKind],
    )
    run_submit.add_argument(
        "--recipe", default=None, help="Recipe name; uses current version"
    )
    ...
    run_submit.add_argument(
        "--parameters",
        default=None,
        help="JSON object or path to a JSON file merged into the materialized recipe",
    )
    run_submit.add_argument(
        "--spec",
        default=None,
        help="YAML/JSON object or file for command-based training/evaluation submissions",
    )
