...
from cli.worker import run_worker_from_args
...
def cmd_web(args: argparse.Namespace) -> None:
    ...
    web_dir = (
        ...
        if args.web_dir
        else Path(__file__).resolve().parents[2] / 'dj-panel-web'
    )
...
def cmd_workspace_create(args: argparse.Namespace) -> None:
    ...
    base_url = resolve_base_url(args, config)
    payload = {'slug': args.slug, 'name': args.name or args.slug, 'description': args.description, 'ownerName': owner}
...
def cmd_workspace_members_add(args: argparse.Namespace) -> None:
    ...
    payload = {'userName': args.user, 'role': args.role}
    with client(base_url) as http:
...
def cmd_recipe_list(args: argparse.Namespace) -> None:
    ...
    with client(base_url) as http:
        ...
        response.raise_for_status()
        render(response.json(), json_output=args.json, human_renderer=render_recipe_list)
...
def cmd_recipe_show(args: argparse.Namespace) -> None:
    ...
    with client(base_url) as http:
        ...
        render(find_recipe_by_name(http, workspace, args.recipe), json_output=args.json, human_renderer=render_recipe)
...
def cmd_recipe_publish(args: argparse.Namespace) -> None:
    ...
    version_payload = {
        'createdBy': owner,
        'recipeBody': payload['recipeBody'],
        'command': payload['command'],
        'scriptPath': payload['scriptPath'],
        'parameterSchema': payload['parameterSchema'],
        'envTemplate': payload['envTemplate'],
        'executionSpec': payload['executionSpec'],
        'timeoutSeconds': payload['timeoutSeconds'],
    }
    with client(base_url) as http:
        recipe = find_recipe_by_name(http, workspace, args.recipe)
        response = http.post(f"/api/v1/recipes/{recipe['id']}/versions", json=version_payload)
        response.raise_for_status()
        render(response.json(), json_output=args.json, human_renderer=render_recipe_version)
