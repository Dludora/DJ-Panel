...
class LineageQueryRepository:
    def get_current_job_inputs_outputs(self, conn: Connection, job_version_id: str) -> tuple[list[str], list[str]]:
        rows = conn.execute(
            select(job_version_io_mapping.c.dataset_id, job_version_io_mapping.c.io_type).where(
                job_version_io_mapping.c.job_version_id == job_version_id
            )
        ).mappings().all()
        inputs = [row['dataset_id'] for row in rows if row['io_type'] == JobVersionIOType.INPUT.value]
        outputs = [row['dataset_id'] for row in rows if row['io_type'] == JobVersionIOType.OUTPUT.value]
    ...
    def get_current_jobs_for_dataset(self, conn: Connection, dataset_id: str) -> tuple[list[str], list[str]]:
        rows = conn.execute(
            ...
            .where(job_versions.c.is_current.is_(True), job_version_io_mapping.c.dataset_id == dataset_id)
        ).mappings().all()
