...
from dj_panel.app.models.constant import RunSubmissionKind, TaskKind
...
def load_processing_run_spec(raw: str | None) -> ProcessingRunSpec:
    payload = load_structured_arg(raw)
    if not payload:
        raise ValueError("processing run submit requires --spec")
    spec = ProcessingRunSpec.model_validate(payload)
    if spec.process.dj_configs.mode == "workspace_recipe":
        if not spec.process.dj_configs.name:
            raise ValueError("workspace_recipe processing spec requires process.dj_configs.name")
        return spec

    recipe_path = spec.process.dj_configs.path
    if not recipe_path:
        raise ValueError("local_file processing spec requires process.dj_configs.path")
    spec.process.dj_configs.recipe_body = load_recipe_body(Path(recipe_path).expanduser())
    spec.process.dj_configs.path = str(Path(recipe_path).expanduser().resolve())
    return spec


def run_submission_kind(value: str | RunSubmissionKind) -> RunSubmissionKind:
    if value == "processing_pipeline":
        return RunSubmissionKind.PROCESSING
    return RunSubmissionKind(value)
