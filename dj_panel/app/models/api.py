...
from dj_panel.app.models.constant import AssetKind, RunSubmissionKind, TaskKind
...
class RunSubmissionCreateRequest(RequestModel):
    ...
    submission_kind: RunSubmissionKind = Field(
        default=RunSubmissionKind.PROCESSING, alias="submissionKind"
    )
    ...
    @field_validator("submission_kind", mode="before")
    @classmethod
    def normalize_processing_kind(cls, value: Any) -> Any:
        if value == "processing_pipeline":
            return RunSubmissionKind.PROCESSING.value
        return value
...
class ProcessingDjConfigs(ApiModel):
    mode: Literal["workspace_recipe", "local_file"]
    name: Optional[str] = None
...
class ProcessingDatasetRef(ApiModel):
    namespace: str
    name: str


class ProcessingDatasets(ApiModel):
    inputs: list[ProcessingDatasetRef] = Field(default_factory=list)


class ProcessingSpec(ApiModel):
    dj_configs: ProcessingDjConfigs = Field(alias="dj_configs")
    datasets: Optional[ProcessingDatasets] = None
    extra_configs: dict[str, Any] = Field(default_factory=dict, alias="extra_configs")
    env: dict[str, Any] = Field(default_factory=dict)
    timeout_seconds: Optional[int] = Field(default=None, alias="timeoutSeconds")


class ProcessingRunSpec(ApiModel):
