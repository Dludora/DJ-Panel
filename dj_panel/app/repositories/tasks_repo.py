...
from dj_panel.app.db.schema import task_artifacts, task_attempts, tasks
from dj_panel.app.db.rows import TaskAttemptRow, TaskRow
from dj_panel.app.models.constant import TaskAttemptStatus, TaskKind, TaskStatus
...
class TaskRepository:
    ...
    def get_task_by_run_submission_id(
        self, conn: Connection, run_submission_id: str
    ) -> Optional[TaskRow]:
        row = (
            conn.execute(
                select(tasks)
                .where(tasks.c.run_submission_id == run_submission_id)
    ...
    def set_attempt_openlineage_run_id(
        self, conn: Connection, attempt_id: str, openlineage_run_id: str
    ) -> TaskAttemptRow:
        now = utc_now()
        conn.execute(
    ...
    def reset_task_for_resume(self, conn: Connection, task_id: str) -> TaskRow:
        conn.execute(
            update(tasks)
    ...
    def cancel_pending_task(
        self, conn: Connection, task_id: str, failure_reason: str
    ) -> TaskRow:
        now = utc_now()
        updated = conn.execute(
            update(tasks)
        ...
        if updated.rowcount == 0:
            raise ValueError("task is not pending and cancelable")
        return self.get_task_by_id(conn, task_id)
