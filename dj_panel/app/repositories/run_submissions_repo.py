...
from dj_panel.app.db.schema import run_submissions
from dj_panel.app.db.rows import RunSubmissionRow
from dj_panel.app.models.constant import RunSubmissionStatus
...
class RunSubmissionRepository:
    ...
    def reset_run_submission_for_resume(
        self, conn: Connection, run_id: str
    ) -> RunSubmissionRow:
        conn.execute(
            update(run_submissions)
            .where(run_submissions.c.id == run_id)
            .values(
                status=RunSubmissionStatus.PENDING.value,
                started_at=None,
                ended_at=None,
                failure_reason=None,
                root_lineage_node_id=None,
            )
        )
        return self.get_run_submission_by_id(conn, run_id)
