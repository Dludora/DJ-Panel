...
For V1, the recommended direction is to make the physical database model asset-oriented while keeping OpenLineage/API compatibility at the service layer.

That means:

- keep `assets` as the primary asset identity table
- keep `asset_versions` as the primary version table
...
- continue to accept OpenLineage `DatasetEvent`, `RunEvent.inputs`, and `RunEvent.outputs`; if no asset-kind facet is present, default to `DATASET`
...
Projected OpenLineage dataset identities generalized into platform asset identities.
...
- A future `models` product view should be backed by the same `assets` table filtered by `asset_kind = MODEL`.
- The public compatibility API may still expose dataset-shaped endpoints for OpenLineage consumers.
...
- `asset_versions` should be able to represent model versions as well, as long as classification and rendering use `assets.asset_kind` plus version facets.
...
- unique (`job_version_id`, `asset_id`, `io_type`)
...
- V1 DJ workers should resolve input locations through the linked `asset_versions.storage_uri`.
...
- `id` UUID PK
- `run_submission_id` UUID FK -> `run_submissions.id`
- `asset_id` UUID FK -> `assets.id` null
