...
```yaml
  /api/v1/workspaces/{workspace_slug}/recipe-runs:
    post:
      summary: Create recipe run
    get:
      summary: List recipe runs

  /api/v1/recipe-runs/{run_id}:
    get:
...
V1 should reuse these implemented APIs where possible and introduce `run-submissions` as the product-facing evolution path.
